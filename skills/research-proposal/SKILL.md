...
6. **PDF Conversion** - Convert Markdown to professional PDF with cover page, clickable TOC, and embedded figures
...
- **Cover page** with document title, "Research Proposal", programme name, and date
- **Clickable Table of Contents** with all H2/H3 sections and a List of Figures, linked to their locations in the PDF
- **Auto-numbered sections** (H2 → `1.`, `2.`, etc.; H3 → `1.1`, `1.2`, etc.)
- **Embedded images** from the `./image/` directory (relative to the Markdown file)
- **Running header** and **page numbers** on content pages
- **Styled tables**, **blockquotes**, and **ordered lists**
- **PDF bookmarks/outline** for navigation in PDF viewers

### Requirements

- Python 3 with `reportlab` installed (`pip install reportlab`)
- Images referenced in the Markdown should exist at their relative paths
- **Bundled fonts** (no installation needed): STIX Two Text and STIX Two Math TTF files are included in `scripts/fonts/` and loaded automatically by the script for native rendering of scientific characters (subscripts ₀-₉, Greek letters α β π σ, arrows →, math operators ≤ ≥ ≈, etc.)

### Example

```bash
python3 scripts/md_to_pdf.py proposal_topic_2026-03-10.md
# -> produces proposal_topic_2026-03-10.pdf
```

### Format Conversion (Alternative)
